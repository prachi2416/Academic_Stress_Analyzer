...
export default function DailyRoutineEditor({ routine, onChange }: Props) {
  ...
  return (
    <div className="bg-white rounded-3xl p-6 sm:p-7 shadow-sm border border-slate-200 reveal-on-scroll">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6 pb-4 border-b border-slate-100">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-bold text-slate-900">Daily Routine &amp; Protected Hours</h3>
        ...
        <div className="flex items-center gap-2">
          {summary.isValid ? (
            <span className="inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200/80 shadow-2xs">
              <ShieldCheck className="w-3.5 h-3.5 text-emerald-600" /> Sleep &amp; Commitments Protected
            </span>
          ) : (
            <span className="inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-full bg-rose-50 text-rose-700 border border-rose-200 shadow-2xs">
              <AlertTriangle className="w-3.5 h-3.5 text-rose-600" /> Schedule Conflict
            </span>
          )}
        </div>
      </div>
      ...
      {errors.length > 0 && (
        <div className="mb-6 p-4 rounded-2xl bg-rose-50 border border-rose-200 text-rose-800 text-xs space-y-1 reveal-on-scroll">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {/* Wake-up Time */}
        <div className="card-hover p-4 rounded-2xl border border-slate-200 bg-slate-50/60 hover:bg-white transition-all duration-200 reveal-on-scroll stagger-1">
          <label className="flex items-center gap-1.5 text-xs font-semibold text-slate-700 mb-2">
            <Clock className="w-3.5 h-3.5 text-amber-500" /> Wake-up Time
          </label>
          <input
            type="time"
            value={routine.wakeTime}
            onChange={(e) => updateField("wakeTime", e.target.value)}
            className="w-full px-3 py-2 text-sm font-semibold font-mono rounded-xl border border-slate-200 bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all"
          />
          <p className="text-[11px] text-slate-400 mt-1.5 font-medium">Morning wake-up time</p>
        </div>

        {/* Sleep Time */}
        <div className="card-hover p-4 rounded-2xl border border-slate-200 bg-slate-50/60 hover:bg-white transition-all duration-200 reveal-on-scroll stagger-2">
          <label className="flex items-center gap-1.5 text-xs font-semibold text-slate-700 mb-2">
        ...
        {/* College Hours */}
        <div className="card-hover p-4 rounded-2xl border border-slate-200 bg-slate-50/60 hover:bg-white transition-all duration-200 reveal-on-scroll stagger-3">
          <label className="flex items-center gap-1.5 text-xs font-semibold text-slate-700 mb-2">
            <School className="w-3.5 h-3.5 text-blue-500" /> College Window
          </label>
          <div className="grid grid-cols-2 gap-2">
            <input
              ...
              className="w-full px-2 py-2 text-xs font-semibold font-mono rounded-xl border border-slate-200 bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all"
            />
            <input
              type="time"
        ...
        {/* Travel & Meals */}
        <div className="card-hover p-4 rounded-2xl border border-slate-200 bg-slate-50/60 hover:bg-white transition-all duration-200 reveal-on-scroll stagger-4">
          <label className="flex items-center gap-1.5 text-xs font-semibold text-slate-700 mb-2">
            <Car className="w-3.5 h-3.5 text-emerald-500" /> Travel &amp; Meals
          </label>
          <div className="grid grid-cols-2 gap-2">
            <div>
              ...
              <input
                ...
                className="w-full px-2.5 py-1.5 text-xs font-mono font-semibold rounded-lg border border-slate-200 bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500"
              />
            </div>
            <div>
      ...
      <div className="mb-6 p-5 rounded-2xl border border-slate-100 bg-slate-50/50 reveal-on-scroll stagger-5">
        <div className="flex items-center justify-between mb-3">
          ...
          {!showAddCommitment && (
            <button
              ...
              className="btn-press inline-flex items-center gap-1 text-xs font-semibold text-indigo-600 hover:text-indigo-700 bg-indigo-50 hover:bg-indigo-100/70 px-3 py-1.5 rounded-xl transition cursor-pointer border border-indigo-100"
            >
        ...
        {showAddCommitment && (
          <div className="mb-3 p-3.5 bg-white rounded-xl border border-indigo-200 shadow-sm flex flex-wrap items-center gap-2.5">
            <input
              type="text"
              placeholder="Commitment name (e.g. Gym, Coaching)"
              value={newCommitmentName}
              onChange={(e) => setNewCommitmentName(e.target.value)}
              className="flex-1 min-w-[160px] text-xs px-3 py-2 rounded-lg border border-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500"
            />
            <div className="flex items-center gap-1.5">
              ...
              <input
                ...
                className="text-xs px-2.5 py-2 rounded-lg border border-slate-200 font-mono focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500"
              />
            ...
            <div className="flex items-center gap-1.5">
              <button
                ...
                className="btn-press text-xs bg-indigo-600 hover:bg-indigo-700 text-white font-semibold px-3.5 py-2 rounded-lg transition shadow-xs cursor-pointer"
              >
                Save
              </button>
              <button
                onClick={() => setShowAddCommitment(false)}
                className="btn-press text-xs bg-slate-100 hover:bg-slate-200 text-slate-600 px-3 py-2 rounded-lg transition cursor-pointer"
              >
                Cancel
              </button>
            </div>
            ...
            {routine.fixedCommitments.map((c) => (
              ...
              >
                ...
                <button
                  ...
                  className="text-slate-400 hover:text-rose-600 transition p-0.5 cursor-pointer rounded-md hover:bg-rose-50"
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-2.5 pt-5 border-t border-slate-100 text-center reveal-on-scroll stagger-6">
        <div className="card-hover p-3 rounded-2xl bg-indigo-50/70 border border-indigo-100/80 transition-all duration-200">
          <p className="text-[10px] font-bold uppercase tracking-wider text-indigo-600">Sleep</p>
          <p className="text-base font-extrabold text-indigo-950 mt-0.5">{summary.sleepHours}h</p>
          <span className="text-[9px] text-indigo-500 font-medium">Protected</span>
        </div>
        <div className="card-hover p-3 rounded-2xl bg-blue-50/70 border border-blue-100/80 transition-all duration-200">
          <p className="text-[10px] font-bold uppercase tracking-wider text-blue-600">College</p>
          <p className="text-base font-extrabold text-blue-950 mt-0.5">{summary.collegeHours}h</p>
          <span className="text-[9px] text-blue-500 font-medium">Protected</span>
        </div>
        <div className="card-hover p-3 rounded-2xl bg-slate-50 border border-slate-200/80 transition-all duration-200">
          <p className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Travel</p>
          <p className="text-base font-extrabold text-slate-800 mt-0.5">{summary.travelHours}h</p>
        ...
        <div className="card-hover p-3 rounded-2xl bg-slate-50 border border-slate-200/80 transition-all duration-200">
          ...
          <p className="text-base font-extrabold text-slate-800 mt-0.5">{summary.mealHours}h</p>
          <span className="text-[9px] text-slate-400 font-medium">Protected</span>
        </div>
        <div className="card-hover p-3 rounded-2xl bg-slate-50 border border-slate-200/80 transition-all duration-200">
          <p className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Fixed</p>
          <p className="text-base font-extrabold text-slate-800 mt-0.5">{summary.fixedCommitmentHours}h</p>
          <span className="text-[9px] text-slate-400 font-medium">Protected</span>
        </div>
        <div className="card-hover p-3 rounded-2xl bg-amber-50/70 border border-amber-200/80 transition-all duration-200">
          <p className="text-[10px] font-bold uppercase tracking-wider text-amber-700">Total Bound</p>
          <p className="text-base font-extrabold text-amber-900 mt-0.5">{summary.totalProtectedHours}h</p>
          <span className="text-[9px] text-amber-600 font-medium">of 24h</span>
        </div>
        <div className="card-hover p-3 rounded-2xl bg-gradient-to-br from-emerald-500 via-emerald-600 to-teal-600 text-white shadow-md shadow-emerald-500/20 col-span-2 sm:col-span-1 border border-emerald-400/30 transition-all duration-200 ring-2 ring-emerald-400/20">
          ...
          <p className="text-xl font-extrabold mt-0.5 text-white drop-shadow-xs">{summary.availableStudyHours}h</p>
